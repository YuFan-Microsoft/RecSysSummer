...
which deepspeed pdsh ssh torchrun rsync
...
- `deepspeed`, `ssh`, `torchrun`, `pdsh` present; **`rsync` MISSING**.
...
python -c "from datasets import get_dataset_config_names as g; \
...
## 2. The three hard problems (and the fixes) — learned the hard way
...
- **LR scaling:** global batch grows `8 × 40 = 320` (was 64). Linear rule
  `LR ≈ 3e-4 × global_batch/1024 ≈ 9e-5` (was 2e-5).
...
Best-effort: missing pieces are logged as `SKIP`, never fatal (training will still
fetch if needed — but with offline mode you want prefetch to succeed).

```bash
# one process per node — the launcher fans this out over ssh, ONLINE:
python scripts/prefetch_hf.py --category Video_Games --base_model Qwen/Qwen3-1.7B
```
...
2. **Prefetch** data+model on all nodes, **ONLINE** (§4b).
3. **Train** across all 40 GPUs via ssh fan-out of `deepspeed --no_ssh --node_rank R`,
...
`GPUS_PER_NODE`, `NCCL_DEBUG`, `HOSTFILE`, `MASTER_ADDR`.
...
- 10 epochs / domain ≈ **~11–12 h**; **all 3 domains ≈ ~33–36 h** (sequential).
...
7. 40 GPUs → global batch 320 → LR 9e-5; wandb project
   `SIDReasoner_Phase1_Distributed_Training` with the 3 domain runs; 10 epochs each,
   one domain at a time (§4c, §5).
