...
def load_retry_module():
    path = Path(__file__).parents[2] / "trainer" / "ppo" / "retry_sampling.py"
    spec = importlib.util.spec_from_file_location("retry_sampling", path)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


class BeamRetryProcessRewardTest(unittest.TestCase):
    def test_valid_trace_receives_full_process_reward(self):
        self.assertEqual(
    ...
    def test_process_variation_does_not_rescue_all_wrong_sid_group(self):
        retry_module = load_retry_module()
        group_ids = np.asarray(["prompt"] * 4, dtype=object)
        sid_rewards = np.zeros(4, dtype=float)
        process_rewards = np.asarray([0.0, 0.5, 0.8, 1.0])

        selected, retry_group_ids, counts = retry_module.classify_retry_groups(
            group_ids,
            sid_rewards,
        ...
        self.assertEqual(retry_group_ids.tolist(), ["prompt"])
        self.assertEqual(counts["all_wrong_groups"], 1)
        self.assertGreater(process_rewards.max(), process_rewards.min())
