...
class BeamProcessMetricsMonitoringTest(unittest.TestCase):
    def test_process_metrics_do_not_raise_a_beam_miss(self):
        for domain in ("Games", "Office", "Industrial"):
            with self.subTest(domain=domain):
                ...
                self.assertEqual(reward["sid_match_reward"], 0.0)
                self.assertEqual(reward["format_reward"], 1.0)
                self.assertEqual(reward["process_reward"], 1.0)

    def test_invalid_process_metrics_do_not_reduce_a_beam_hit(self):
    ...
    def test_valid_metrics_report_full_grounding_and_coverage(self):
        reward = load_reward_module("Games").MyRewardComputer().compute(
            data_source="rec/test",
            solution_str=VALID_REASONING,
            ground_truth="<a_1><b_2><c_3>",
        ...
        self.assertEqual(reward["history_summary_grounding_reward"], 1.0)
        self.assertEqual(reward["future_interests_grounding_reward"], 1.0)
        self.assertEqual(reward["history_reference_coverage"], 1.0)
        self.assertEqual(reward["process_reward"], 1.0)
